...
int fs_read(int fd, void *buf, size_t count)
{
	...
	*/
	...
	else if (countOfBytesToRead < BLOCK_SIZE) // reading from a single block
	{
	...
	else if (countOfBytesToRead > BLOCK_SIZE) // reading from multiple blocks
	{
		...
		if (fdArray[fd].file_offset == 0) // All blocks are read in entirety except for the last one = full
		{
			...
			{
				if (countOfBytesToRead < BLOCK_SIZE) // The last block's case when not read in entirety
				{
					// printf("Read CP10\n");
					...
					memcpy(readBuf, endBuff + bounceBufOffSet, countOfBytesToRead);
					numBytesRead += countOfBytesToRead;
					fdArray[fd].file_offset += countOfBytesToRead;
					break;
				}
